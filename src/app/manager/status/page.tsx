        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 xl:px-8">
          <div className="mb-6 sm:mb-8">
            <div className="flex items-center justify-between mb-3 sm:mb-4">
              <div className="flex items-center">
                <Users className="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 text-blue-600 mr-2 sm:mr-3" />
                <div>
                  <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900">Employee Status</h1>
                  <p className="text-sm sm:text-base text-gray-600 mt-1">Real-time status of all employees</p>
                </div>
              </div>
              <button
                onClick={handleRefresh}
                disabled={refreshing}
                className="flex items-center px-3 sm:px-4 py-2 sm:py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-sm sm:text-base"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
            {employees.map((employee) => (
              <div key={employee._id} className="bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 hover:shadow-md transition-shadow">
                ...
                <div className="space-y-2 sm:space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${getStatusColor(employee)}`}>
                  ...
                  {employee.isCheckedIn && (
                    <>
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs sm:text-sm text-gray-900">{formatTime(employee.clockInTime)}</span>
                      </div>
                      
                      {employee.lunchStartTime && (
                        <div className="flex items-center justify-between">
                          <span className="text-xs sm:text-sm text-gray-600">Lunch Start:</span>
                          <span className="text-xs sm:text-sm text-gray-900">{formatTime(employee.lunchStartTime)}</span>
                        </div>
                      )}
                      
                      {employee.lunchEndTime && (
                        <div className="flex items-center justify-between">
                          <span className="text-xs sm:text-sm text-gray-600">Lunch End:</span>
                          <span className="text-xs sm:text-sm text-gray-900">{formatTime(employee.lunchEndTime)}</span>
                        </div>
                      )}
                      
                      <div className="flex items-center justify-between">
                        <span className="text-xs sm:text-sm text-gray-600">Hours Today:</span>
                        <span className="text-xs sm:text-sm text-gray-900 font-medium">{formatHours(employee.totalWorkingHours)}</span>
                      </div>
                    </>
                  )}
                  ...
                  {employee.status === 'completed' && employee.clockOutTime && (
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs sm:text-sm text-gray-900">{formatTime(employee.clockOutTime)}</span>
                    </div>
                  )}
                </div>
              </div>
          ...
          {employees.length === 0 && !loading && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">No employee data available at the moment.</p>
            </div>
          )}
        </div>
      </div>
